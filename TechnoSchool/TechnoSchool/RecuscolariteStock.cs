...
{
    public partial class RecuscolariteStock : Form
    {
        // ---------------
        ...
        // Récupération des infos concernant le réçu
        public class Maxcaisse
        {
            ...
            public string RecuClick { get; set; }
            public Maxcaisse(Label session, Label numrecu, Label nom, Label dates, Label heure, Label classe, Label motif, Label montant, Label montlettre, Label matricule, Label reste, string recuClick)
            {
            ...
            public void maxcaisse()
            {
                ...
                connection42.Open();
                string res2 = "select monverse,matricule,session,dateverse,heure from caissescolarite where numrecu=" + RecuClick + "";
                ...
                while (reader42.Read())
                {
                    ...
                    Numrecu.Text = RecuClick;
                    Montlettre.Text = converti(int.Parse(reader42.GetValue(0).ToString()))+ " franc cfa";
                }
                reader42.Close();
                string montinscript = "";
                string se = "select scolarite.session,scolarite.reste,scolarite.id_classe,scolarite.matricule,eleves.matricule,eleves.nom_eleve,eleves.prenom_eleve,classe.id_classe,classe.nom_classe,scolarite.montscolarite from scolarite inner join eleves on scolarite.matricule=eleves.matricule inner join classe on scolarite.id_classe=classe.id_classe where scolarite.matricule=@matricule AND scolarite.session=@session";
                ...
                while (reader2.Read())
                {
                    ...
                    montinscript = reader2.GetValue(9).ToString();

                }
                reader2.Close();
                para.Clear();
                // somme de tout les versement du numéro de reçu cliqué et tout les versement precedent
                string se2 = "select monverse,sum(monverse) as nbresom from caissescolarite where matricule=@matricule AND session=@session AND numrecu <= @recu AND typeversement=@type";
                Dictionary<string, object> para2 = new Dictionary<string, object>();
                para2.Add("@matricule", Matricule.Text);
                para2.Add("@session", Session.Text);
                para2.Add("@recu", RecuClick);
                para2.Add("@type", "scolarite");
                var connect2 = new MySqlConnection(connectionstring);
                connect2.Open();
                var command = new MySqlCommand(se2, connect2);
                ...
                while (reader3.Read())
                {
                    string somme = reader3.GetValue(1).ToString();
                    long rest = (long.Parse(montinscript) - long.Parse(somme));
                    Reste.Text = rest.ToString();
                    if (rest > 0)
                    {
                        Motif.Text = "SCOLARITE";
                    }
                    else if (rest == 0)
                    {
                        Motif.Text = "SOLDE SCOLARITE";
                    }
                }
            }
        }
        public RecuscolariteStock(string recu)
        {
            InitializeComponent();
            connexionDB();
            Infoschool ecol = new Infoschool(label5, label6, label7, label9, label8, pictureBox1, label10);
            ecol.Infos();
            Maxcaisse recus = new Maxcaisse(label27, label30, label31, label32, label33, label34, label35, label36, label37, label38, label39, recu);
            recus.maxcaisse();
        }
