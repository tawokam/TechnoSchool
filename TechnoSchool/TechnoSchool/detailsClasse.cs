...
{
    public partial class detailsClasse : Form
    {
        // ---------------
        ...
        public class Detail
        {
            private Label nom, inscription, scolarite, section, modif, creer;
            private Label Nom { set; get; }
            private Label Inscription { set; get; }
            private Label Scolarite { set; get; }
            private Label Section { set; get; }
            private Label Modif { set; get; }
            private Label Creer { set; get; }
            public Detail(Label nom, Label inscription, Label scolarite, Label section, Label modif, Label creer)
            {
                this.Nom = nom; this.Inscription = inscription; this.Scolarite = scolarite; this.Section = section; this.Modif = modif;
                this.Creer = creer;
            }
            ...
            public void detailinfo(int idline)
            {
                ...
                while (reader.Read())
                {
                    ...
                    Inscription.Text = reader.GetValue(3).ToString();
                    Scolarite.Text = reader.GetValue(2).ToString();
                    Section.Text = reader.GetValue(6).ToString();
                    Modif.Text = string.Format("{0:dd / MM / yyyy}", reader.GetValue(8));
                    Creer.Text = string.Format("{0:dd / MM / yyyy}", reader.GetValue(7));
                }
                reader.Close();
            }
        }
        ...
        public detailsClasse(int idligne)
        {
            InitializeComponent();
            connexionDB();
            Detail newdet = new Detail(label10, label9, label7, label8, labelPhone2, labelPhone1);
            newdet.detailinfo(idligne);
        }

        private void detailsClasse_Paint(object sender, PaintEventArgs e)
        {
