...
{
    public partial class ChoixElevePrDossier : Form
    {
        // ---------------
        ...
        // Class pour afficher la liste des élèves inscripts pour acceder a leurs dossier
        public class ListEleve
        {
            ...
            // constructeur
            public ListEleve(DataGridView tableauDonnees, string nom)
            {
            ...
            public void listversement(DataGridView datagridview)
            {
                ...
                if (Nom == "null")
                {
                    requete = "Select matricule,nom_eleve,prenom_eleve from eleves order by nom_eleve asc";
                    command = new MySqlCommand(requete, connection);
                }
                else
                {
                    requete = "Select matricule,nom_eleve,prenom_eleve from eleves where nom_eleve LIKE @nom order by nom_eleve asc";
                ...
                while (reader.Read())
                {
                    datagridview.Rows.Add(reader.GetValue(0).ToString(), reader.GetValue(1).ToString(), reader.GetValue(2));
                }
                reader.Close();
                connection.Close();
            }
        ...
        public ChoixElevePrDossier()
        {
            InitializeComponent();
            connexionDB();
            ListEleve eleve = new ListEleve(tableaudonnees, "null");
        ...
        private void ChoixElevePrDossier_Paint(object sender, PaintEventArgs e)
        {
        ...
        private void nomSchool_KeyUp(object sender, KeyEventArgs e)
        {
            string nom = nomSchool.Text;
            ListEleve eleve = new ListEleve(tableaudonnees, nom);
        }

        private void tableaudonnees_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            // verifie si au moins une ligne existe 
            if (tableaudonnees.Rows.Count == 0)
            {
                string messag = "Aucun élève séléctionné. ";
                string titre = "Dossier des élèves";
                // Programation des bouton de la boite de message
            ...
            else
            {
                ...
                RapportDossierElevecs details = new RapportDossierElevecs(nom);
                details.ShowDialog();
            }
        }
    }
