...
{
    public partial class deletescolarite : Form
    {
        // ---------------
        ...
        public class Listverse
        {
            ...
            public void listversement(DataGridView datagridview)
            {
                ...
                while (reader.Read())
                {
                    datagridview.Rows.Add(reader.GetValue(0).ToString(), reader.GetValue(1).ToString(), string.Format("{0:dd / MM / yyyy}", reader.GetValue(2)), reader.GetValue(3).ToString());
                }
                reader.Close();
            }
            ...
            public void infoverse()
            {
                ...
                try
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        {
                            // Somme de tous les versements dans la caisse pour scolarité et pour le matricule et la session
                            long sommcaiss = 0;
                            ...
                            // Séléction de la ligne scolarite et modification de la ligne en question
                            string seinscript = "SELECT montscolarite,montverse,reste From scolarite where matricule=@matricule AND session=@session";
                            ...
                            while (reader.Read())
                            {
                                ...
                                // Verifions si le montant versé est egal a zero XXX
                                // Si c'est le cas supprimer la ligne en question dans la table scolarite XXX
                                // Si non modifions la ligne conserné XXX 
                                // algorithme modifier : lors de l'inscription ajouter automatiquement la ligne scolarité avec un montant versé de 0 franc cfa

                                    // modification de la ligne dans la table inscription
                                    string inser = "UPDATE scolarite SET montverse=@montv, reste=@reste, statut=@statut where matricule=@matricule AND session=@session";
                    ...
                    string messagok = "Versement supprimer avec succès";
                    string titreok = "Suppression";
                    // Programation des bouton de la boite de message
                ...
                {
                    string messagok = "Nous nous sommes heurtés à un problème lors de la suppression du versement "+error;
        ...
        public deletescolarite(string matricule, string session)
        {
            InitializeComponent();
            connexionDB();
        ...
        private void deletescolarite_Paint(object sender, PaintEventArgs e)
        {
