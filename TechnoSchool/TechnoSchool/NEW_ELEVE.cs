...
{
    public partial class NEW_ELEVE : Form
    {
        // ---------------
        ...
        public class designDatagridview
        {
            ...
            public void affichDonnees(DataGridView datagridview)
            {
                ...
                datagridview.Columns[4].Name = "Sexe";
                ...
                string requete = "Select nom_eleve,prenom_eleve,matricule,sexe from eleves order by nom_eleve asc";
            ...
            public void affichDonneesdatagrid(DataGridView datagridview, int numbtn, int nbreligneParPage)
            {
                ...
                datagridview.AllowUserToAddRows = false;
                // couleur des entetes de colonnes
                string requete = "Select nom_eleve,prenom_eleve,matricule,sexe from eleves order by nom_eleve asc limit " + nbreligneParPage + " offset " + lignedebut + "";
                command = new MySqlCommand(requete, connection);
                command.Prepare();
                reader = command.ExecuteReader();
                datagridview.Rows.Clear();
                ...
                while (reader.Read())
                {
                    datagridview.Rows.Add(number.ToString(), "" + reader.GetValue(2), reader.GetValue(0).ToString(), reader.GetValue(1).ToString(), reader.GetValue(3).ToString());
                    number++;
                }
                reader.Close();
            ...
            public int Compteligne(Label labeltete, Panel numpclob, Label backPage1, Label precedentPage, Label suivantPage, Label dernierPage, Label btnPage1, Label btnPage2, Label btnPage3, Label btnPage4, Label btnPage5, int numPageClick, int nbreLignePage)
            {
                ...
                if (nbrePage <= 1)
                {
                    ...
                    labeltete.Text = "1 / 1 page        " + nbreligne + " élève(s)  enregistré(s)";
                ...
                else
                {
                    ...
                    labeltete.Text = numPageClick + " / " + newNbrePage + " page(s)        " + nbreligne + " élève(s)  enregistré(s)";
        ...
        public class designDatagridviewSearch
        {
            ...
            public void affichDonnees(DataGridView datagridview)
            {
                ...
                datagridview.AllowUserToAddRows = false;
                // couleur des entetes de colonnes
                string requete = "Select nom_eleve,prenom_eleve,matricule,sexe From eleves where nom_eleve like @nom OR prenom_eleve like @prenom OR matricule like @matricule order by nom_eleve asc";
                command = new MySqlCommand(requete, connection);
            ...
            public void affichDonneesdatagrid(DataGridView datagridview, int numbtn, int nbreligneParPage)
            {
                ...
                datagridview.AllowUserToAddRows = false;
                // couleur des entetes de colonnes
                string requete = "Select nom_eleve,prenom_eleve,matricule,sexe From eleves where nom_eleve like @nom OR prenom_eleve like @prenom OR matricule like @matricule order by nom_eleve asc limit " + nbreligneParPage + " offset " + lignedebut + "";
                command = new MySqlCommand(requete, connection);
                Dictionary<string, object> parametre = new Dictionary<string, object>();
                parametre.Add("@nom", "%" + Nom + "%");
                parametre.Add("@prenom", "%" + Nom + "%");
                ...
                while (reader.Read())
                {
                    datagridview.Rows.Add(number.ToString(), "" + reader.GetValue(2).ToString(), reader.GetValue(0).ToString(), reader.GetValue(1).ToString(), reader.GetValue(3).ToString());
                    number++;
            ...
            public int Compteligne(Label labeltete, Panel numpclob, Label backPage1, Label precedentPage, Label suivantPage, Label dernierPage, Label btnPage1, Label btnPage2, Label btnPage3, Label btnPage4, Label btnPage5, int numPageClick, int nbreLignePage)
            {
                ...
                {
                    // Une seul page trouvé
                    labeltete.Text = "1 / 1 page        " + nbreligne + " élève(s)  enregistré(s)  Nom / Prenom / Matricule qui contient : " + Nom + " ";
                    numpclob.Visible = false;
                    btnPage1.Visible = false;
                    btnPage2.Visible = false;
                    btnPage3.Visible = false;
                ...
                else
                {
                    ...
                    // Plusieurs pages trouvé
                    numpclob.Visible = true;
                    labeltete.Text = numPageClick + " / " + newNbrePage + " page(s)        " + nbreligne + " élève(s)  enregistré(s) Nom / Prenom / Matricule qui contient : " + Nom + " ";
                    // remplissage de mon dictionnaire
                    for (int b = 1; b <= newNbrePage; b++)
                    {
                        numPage.Add(b);
        ...
        public class SearchId
        {
            ...
            // Constructeur
            public SearchId(string matricule)
            {
        ...
        public class deleteData
        {
            ...
            // construction
            public deleteData(string matricule)
            {
                this.Matricule = matricule;
                delete();
            }
            ...
            private void delete()
            {
                ...
                string messagconf = "Souhaitez-vous réellement supprimer l'élève ayant le matricule " + Matricule + " ?. La suppression d'un élève entraine la suppression définitive de toutes les données de cette élève";
                ...
                if (result == DialogResult.Yes)
                {
                    try
                    {
                        var connection = new MySqlConnection(connectionstring);
                        connection.Open();
                        // verifier si l'élève séléctionné a été inscript au moins une fois
                        int nbreinscript = 0;
                        string ve = "SELECT count(matricule) as nbreinscript from inscription where matricule='" + Matricule + "'";
                        ...
                        if (nbreinscript == 0)
                        {
                            string requete = "DELETE FROM eleves where matricule=@matricule";
                            Dictionary<string, object> parametre = new Dictionary<string, object>();
                            parametre.Add("@matricule", Matricule);
                            command = new MySqlCommand(requete, connection);
                            ...
                            {
                                string messag = "Elève supprimé avec succès. ";
                            ...
                            {
                                string messag = "Echec de suppression de l'élève. Veuillez réessayer. ";
                        ...
                        {
                            string messag = "Echec de suppression de l'élève. Le dossier d'un élève ne peut etre supprimer. ";
                            string titre = "Echec de Suppression";
                            // Programation des bouton de la boite de message
                            MessageBox.Show(messag, titre, MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    ...
                    {
                        string messag = "Nous nous sommes heurtés à un problème lors de la suppression de cette élève. ";
                        string titre = "Echec de Suppression";
                        // Programation des bouton de la boite de message
                        MessageBox.Show(messag, titre, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
        ...
        private void NEW_ELEVE_Paint(object sender, PaintEventArgs e)
        {
        ...
        private void NEW_ELEVE_Load(object sender, EventArgs e)
        {
        ...
        private void textBox1_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            designDatagridviewSearch tabschool = new designDatagridviewSearch(tableaudonnees, nom);
            this.tableaudonnees.EnableHeadersVisualStyles = false;
            tableaudonnees.ColumnHeadersDefaultCellStyle.BackColor = Color.Blue;
            tableaudonnees.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
            tabschool.Compteligne(tetedonnees, panel8, backPage1, precedentPage, suivantPage, dernierPage, btnPage1, btnPage2, btnPage3, btnPage4, btnPage5, 1, 15);
            ...
            // Respecter l'ordre d'appel des differentes méthode
            int numcont = int.Parse(btnPage1.Text);
            designDatagridviewSearch bv = new designDatagridviewSearch(tableaudonnees, nom);
            bv.Compteligne(tetedonnees, panel8, backPage1, precedentPage, suivantPage, dernierPage, btnPage1, btnPage2, btnPage3, btnPage4, btnPage5, numcont, 15);
            bv.affichDonneesdatagrid(tableaudonnees, numcont, 15);
            if (nom == "")
            {
        ...
        private void label3_Click(object sender, EventArgs e)
        {
            ...
            {
                string messag = "Aucun élève séléctionné pour la suppression ";
                string titre = "Suppression";
                // Programation des bouton de la boite de message
                MessageBox.Show(messag, titre, MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            else
            {
                ...
                deleteData supprimer = new deleteData(matricule);
            }
        }
        ...
        private void label13_Click(object sender, EventArgs e)
        {
            ...
            if (tableaudonnees.Rows.Count == 0)
            {
                string messag = "Aucun élève séléctionné pour plus de détails.";
            ...
            {
                string matricule = (tableaudonnees.CurrentRow.Cells["matricule"].Value).ToString();
                SearchId newde = new SearchId(matricule);
                int valeur = newde.idLigne(0);
                detailsEleve details = new detailsEleve(valeur);
                details.ShowDialog();
            }
        }
        ...
        private void label2_Click(object sender, EventArgs e)
        {
            ...
            if (tableaudonnees.Rows.Count == 0)
            {
                string messag = "Aucun élève séléctionné pour la modification. ";
            ...
            else
            {
                ...
                int valeur = newde.idLigne(0);
                EditEleve details = new EditEleve(valeur);
                details.ShowDialog();
            }
        }
