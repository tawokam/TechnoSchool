    ...
    public partial class NEW_USERS : Form
    {
        ...
        public class designDatagridview
        {
            ...
            public void affichDonnees(DataGridView datagridview)
            {
                ...
                string requete = "Select employer.nom_employer, employer.adresseMail_employer, employer.telephone1_employer,employer.id_fonction,employer.specialitediplome,fonction.id_fonction,fonction.nom_fonction from employer inner join fonction on employer.id_fonction=fonction.id_fonction order by nom_employer asc";
            ...
            public void affichDonneesdatagrid(DataGridView datagridview, int numbtn, int nbreligneParPage)
            {
                ...
                datagridview.Columns[3].Name = "Téléphone";
                datagridview.Columns[4].Name = "Fonction";
                datagridview.Columns[5].Name = "Spécialité"; 
                ...
                // couleur des entetes de colonnes
                string requete = "Select employer.nom_employer, employer.adresseMail_employer, employer.telephone1_employer,employer.id_fonction,employer.specialitediplome,fonction.id_fonction,fonction.nom_fonction from employer inner join fonction on employer.id_fonction=fonction.id_fonction order by nom_employer asc limit " + nbreligneParPage + " offset " + lignedebut + "";
                command = new MySqlCommand(requete, connection);
                command.Prepare();
                reader = command.ExecuteReader();
                datagridview.Rows.Clear();
                ...
                while (reader.Read())
                {
                    datagridview.Rows.Add(number.ToString(), "" + reader.GetValue(0).ToString(), reader.GetValue(1).ToString(), reader.GetValue(2).ToString(), reader.GetValue(6).ToString(), reader.GetValue(4).ToString());
                    number++;
                }
                reader.Close();
            ...
            public int Compteligne(Label labeltete, Panel numpclob, Label backPage1, Label precedentPage, Label suivantPage, Label dernierPage, Label btnPage1, Label btnPage2, Label btnPage3, Label btnPage4, Label btnPage5, int numPageClick, int nbreLignePage)
            {
                ...
                if (nbrePage <= 1)
                {
                    ...
                    labeltete.Text = "1 / 1 page        " + nbreligne + " employé(s)  enregistré(s)";
                ...
                else
                {
                    ...
                    labeltete.Text = numPageClick + " / " + newNbrePage + " page(s)        " + nbreligne + " employé(s)  enregistré(s)";
        ...
        public class SearchId
        {
            // Propriétés
            private string nom;
            private long telephon1;
            private DataGridView tableauDonnees;
            // Accesseurs
            private string Nom { set; get; }
            private long Telephone1 { set; get; }
           
            // Constructeur
            public SearchId(string nom, long telephone1)
            {
                this.Nom = nom; this.Telephone1 = telephone1;
            }
            ...
            public int idLigne(int idline)
            {
                ...
                connection.Open();
                string requete = "SELECT id_employer FROM employer where nom_employer=@nom AND telephone1_employer=@phone1";
                Dictionary<string, object> parametre = new Dictionary<string, object>();
                parametre.Add("@nom", Nom);
                parametre.Add("@phone1", Telephone1);
                command = new MySqlCommand(requete, connection);
        ...
        public class deleteData
        {
            ...
            // construction
            public deleteData(string nom, string telephone1)
            {
                this.Nom = nom; this.Telephone1 = telephone1;
                delete();
            }
            ...
            private void delete()
            {
                string messag = "Souhaitez-vous réellement suprimé cet employé. ";
                string titre = "Suppression";
                // Programation des bouton de la boite de message
                DialogResult res =  MessageBox.Show(messag, titre, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                if(res == DialogResult.Yes)
                {
                    string requete = "DELETE FROM employer where nom_employer=@nom AND telephone1_employer=@telephone";
                    Dictionary<string, object> parametre = new Dictionary<string, object>();
                    parametre.Add("@nom", Nom);
                    parametre.Add("@telephone", Telephone1);
                    var connection = new MySqlConnection(connectionstring);
                    connection.Open();
                    ...
                    {
                        string messag2 = "employé supprimé avec succès. ";
                    ...
                    {
                        string messag2 = "Echec de suppression de l'employé. Veuillez réessayer. ";
                        string titre2 = "Echec de Suppression";
                        // Programation des bouton de la boite de message
        ...
        private void NEW_USERS_Paint(object sender, PaintEventArgs e)
        {
        ...
        {
            Addemployer newempl = new Addemployer();
            newempl.Show();
        }
        ...
        private void label13_Click(object sender, EventArgs e)
        {
            ...
            {
                string messag = "Aucun employé séléctionné pour plus de détails. ";
                string titre = "Suppression";
                // Programation des bouton de la boite de message
                MessageBox.Show(messag, titre, MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            else
            {
                ...
                long telephone = long.Parse((tableaudonnees.CurrentRow.Cells["Téléphone"].Value).ToString());
                SearchId newde = new SearchId(nom, telephone);
                int valeur = newde.idLigne(0);
                detailEmployer details = new detailEmployer(valeur);
                details.ShowDialog();
            }
        ...
        private void label3_Click(object sender, EventArgs e)
        {
            ...
            if (tableaudonnees.Rows.Count == 0)
            {
                string messag = "Aucun employé séléctionné pour la suppression. ";
                string titre = "Suppression";
                // Programation des bouton de la boite de message
                MessageBox.Show(messag, titre, MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            ...
            {
                string nom = (tableaudonnees.CurrentRow.Cells["nom"].Value).ToString();
                string telephone = (tableaudonnees.CurrentRow.Cells["Téléphone"].Value).ToString();
                deleteData supprimer = new deleteData(nom, telephone);
            }
        ...
        private void label2_Click(object sender, EventArgs e)
        {
            ...
            if (tableaudonnees.Rows.Count == 0)
            {
                string messag = "Aucun employé séléctionné pour la modification. ";
            ...
            else
            {
                ...
                SearchId newde = new SearchId(nom, telephone);
                int valeur = newde.idLigne(0);
                EditEmployer details = new EditEmployer(valeur);
                details.Show();
            }
